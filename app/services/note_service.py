...
class NoteService:
    ...
    def user_can_read(self, user_id: int, note: Note) -> bool:
        ...
        if self.shares.has_note_share(note_id=note.id, user_id=user_id):
            return True
        if self.shares.has_any_label_share(
    ...
    def _set_labels(
        self, owner_id: int, note_id: int, label_ids: list[int]
    ) -> None:
    ...
    def list_visible(self, user_id: int) -> list[Note]:
        ...
        ids_by_label = self.labels.list_note_ids_by_label_ids(
            label_ids=shared_label_ids
        )
    ...
    def update(self, user_id: int, note_id: int, payload: NoteUpdate) -> Note:
        ...
        if not note:
            raise HTTPException(
                404, "Actualización fallida de nota: No encontrada"
            )
        if not self.user_can_edit(user_id, note):
            raise HTTPException(
                403,
                "Actualización fallida de nota: Sin permiso para editar nota",
            )
    ...
    def delete(self, user_id: int, note_id: int) -> None:
        ...
        if not note:
            raise HTTPException(
                404, "Eliminación fallida de nota: No encontrada"
            )
        if note.owner_id != user_id:
            raise HTTPException(
                403, "Eliminación fallida de nota: No eres dueño de la nota"
            )
